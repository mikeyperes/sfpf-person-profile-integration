<?php namespace sfpf_person_website;
...
function ajax_load_github_versions() {
    ...
    // Also add main branch as an option
...
function ajax_force_update_check() {
    ...
    delete_site_transient( 'sfpf_gu_version_' . md5( $plugin_basename ) );
    delete_site_transient( 'sfpf_gu_repo_' . md5( $plugin_basename ) );
    delete_site_transient( 'sfpf_github_ver_' . md5( $github_repo . $github_branch ) );
    ...
    $new_version = sfpf_get_github_version_fresh( $github_repo, $github_branch );
 ...
 */
function sfpf_get_github_version_fresh( $repo, $branch = 'main' ) {
    ...
    if ( preg_match( '/^[\s\*]*Version:\s*(.+)$/mi', $body, $matches ) ) {
        ...
        set_site_transient( 'sfpf_github_ver_' . md5( $repo . $branch ), $version, 30 * MINUTE_IN_SECONDS );
        return $version;
    }
...
function ajax_direct_update_plugin() {
    ...
    $temp_dir = get_temp_dir() . 'sfpf-update-' . time();
    $temp_zip = $temp_dir . '/github-download.zip';
    ...
    // Find the extracted folder (GitHub names it repo-branch, e.g., sfpf-person-profile-integration-main)
    $extracted_folders = glob( $extract_dir . '/*', GLOB_ONLYDIR );
    if ( empty( $extracted_folders ) ) {
        sfpf_delete_directory( $temp_dir );
        wp_send_json_error( 'No folder found in extracted zip' );
    ...
    $source_folder = $extracted_folders[0]; // This is sfpf-person-profile-integration-main
...
function sfpf_get_plugin_data() {
    ...
    if (!file_exists($main_plugin_file) || !is_file($main_plugin_file) || !is_readable($main_plugin_file)) {
        // Fallback to SFPF_PLUGIN_DIR constant
        $main_plugin_file = SFPF_PLUGIN_DIR . 'initialization.php';
    }
    ...
    if (!file_exists($main_plugin_file)) {
        return [
            ...
            'AuthorURI' => 'https://seoforpublicfigures.com',
        ];
    }
...
function sfpf_display_plugin_info() {
    $plugin_data = sfpf_get_plugin_data();
    ...
    <div class="sfpf-card">
        <div class="sfpf-card-header">
            ...
            <h3>SFPF Person Profile Integration - Plugin Info</h3>
        </div>
        <div class="sfpf-card-content">
            ...
            <div style="margin-bottom: 15px; padding: 15px; background: <?php echo $update_available ? '#fcf0f1' : '#edfaef'; ?>; border: 1px solid <?php echo $update_available ? '#d63638' : '#00a32a'; ?>; border-radius: 6px;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <div>
                        ...
                        <span id="sfpf-latest-version" style="font-size: 16px; font-weight: bold; color: <?php echo $update_available ? '#d63638' : '#00a32a'; ?>;">
    ...
    jQuery(document).ready(function($) {
        ...
        $('#sfpf-load-versions').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $select.empty();
                        $select.append('<option value="">-- Select Version --</option>');
        ...
        $('#sfpf-download-version').on('click', function() {
            ...
            $btn.prop('disabled', true).text('⏳ Preparing...');
            $status.html('<span style="color: #666;">Downloading ' + version + ' from GitHub...</span>');
